...
Database.prototype.$init = function(callback){
  ...
  function init(err, infoList){
    ...
    infoList.forEach(function(info){
      var table = self.$addTable(info.name, info.indexes, false);
...
Database.prototype.$addTables = function(_tables){
  var tables = _tables||[];
  tables.forEach(this.$addTable, this);
  return this;
};

Database.prototype.$addTable = function(name, indexes, _virtual){
  var virtual = _virtual === false ? _virtual : true;
  var table = this.$tables[name];
  if (table == null){
    table = this.$tables[name] = new Table(name, this);
...
Database.prototype.run = function(_table, _query){
  var table, query, promise, runner, ctx;
  if (typeof _table === 'function'){
    query = _table;
    table = r;
  } else {
    query = _query;
    table = this.$tables[_table];
    if (table == null){
